...
vac_id integer not null auto_increment,
...
references recruiting_classification (dc_id);
...
references recruiting_recruiter (dc_id);

alter table recruiting_candidate
add constraint FK_CLASS_CANDIDATE_STATUS
foreign key (candidate_status)
references recruiting_candidate_status (dc_id);

alter table recruiting_feedback
add constraint FK_CANDIDATE_FEEDBACK
foreign key (candidate_id)
references recruiting_candidate (candidate_id);
...
foreign key (dc_id)
references core_branch_location (dc_id);

alter table recruiting_ref_branchlocation_vacancy
add constraint FK_BRANCHLOCATION_VACANCY
foreign key (vac_id)
references recruiting_vacancy (vac_id);
...
foreign key (dc_id)
references recruiting_career_level (dc_id);
...
references recruiting_vacancy_status (dc_id);
